    ...
    >
      ...
      <span
        className={[
          ...
          solid
            ? isActive ? "text-[#2596be]" : "text-[#006da4]"
            : isActive ? "text-[#2596be]" : "text-[#006da4]",
        ].join(" ")}
      ...
      {/* Back face */}
      <span
        className="absolute inset-0 flex items-center justify-center text-[14px] font-semibold tracking-[0.06em] uppercase select-none text-[#1a56db] transition-all duration-300 ease-in-out origin-top translate-y-full opacity-0 group-hover:translate-y-0 group-hover:opacity-100"
        style={{ backfaceVisibility: "hidden" }}
      >
        {label}
      </span>
      ...
      {/* Active underline */}
      {isActive && (
        <span className="absolute bottom-0 left-3 right-3 h-[2px] rounded-full bg-[#006da4]" />
      )}
    </Link>
  );
}
...
export default function Navbar() {
  ...
  const navH     = 72; /* px — keep in sync with layout marginTop */

  return (
    <>
      {/* ── TOP RIBBON ─────────────────────────────────────────── */}
      <div
        className="fixed top-0 left-0 right-0 z-[60] flex items-center justify-evenly px-6 md:px-14 transition-all duration-300 overflow-hidden bg-[#f4f7fb] border-b border-gray-200"
      ...
      >
        ...
        <div className="flex items-center gap-5">
          <a
            ...
            className="flex items-center gap-1.5 text-[11px] text-gray-500 hover:text-[#1a56db] transition-colors whitespace-nowrap"
          >
        ...
        <div className="flex items-center gap-1">
          {SOCIAL_LINKS.map((s) => (
            <a
              ...
              target="_blank" rel="noopener noreferrer"
              aria-label={s.label}
              className="w-6 h-6 rounded-full flex items-center justify-center text-gray-400 border border-gray-200 bg-white hover:text-white hover:bg-[#1a56db] hover:border-[#1a56db] transition-all duration-200"
            >
              {s.icon}
            </a>
          ))}
      ...
      <header
        className={[
          ...
          solid
            ? "bg-white/97 backdrop-blur-xl shadow-[0_1px_0_0_rgba(0,0,0,0.07),0_4px_20px_rgba(0,0,0,0.04)]"
            : "bg-transparent",
        ].join(" ")}
        style={{
          top: `${ribbonH}px`,
      ...
      >
        ...
        {/* Desktop nav */}
        <nav className="hidden md:flex gap-4.5 items-center h-full ml-auto">
          {NAV_LINKS.map((link) => (
            <FlipLink
              key={link.label}
              label={link.label}
            ...
            return (
              <Link
                ...
                className={[
                  ...
                  active ? "text-[#1a56db]" : "text-gray-800 hover:text-[#1a56db]",
                ].join(" ")}
                style={{
          ...
          <div className="flex items-center gap-2 mt-4">
            {SOCIAL_LINKS.map((s) => (
              <a
                ...
                target="_blank" rel="noopener noreferrer"
                aria-label={s.label}
                className="w-7 h-7 rounded-full flex items-center justify-center text-gray-400 border border-gray-200 hover:text-white hover:bg-[#1a56db] hover:border-[#1a56db] transition-all duration-200"
              >
                {s.icon}
              </a>
            ))}
